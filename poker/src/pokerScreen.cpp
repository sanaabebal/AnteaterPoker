...
static string getCardFilePath(const Card& card) {
    ...
    string path = mainDeckRef[card.getSuit()][card.getValue()];
...
int main(int argc, char* argv[]) {
    ...
    vector<PLAYER> finalPlayersList;
    for (size_t i = 0; i < mockData.seats.size(); ++i) {
        PLAYER uiSeat;
        snprintf(uiSeat.name, sizeof(uiSeat.name), "Player %d", (int)i+1);
        uiSeat.score = mockData.seats[i].score;
        uiSeat.playerNum = (int)i;
        // uiSeat.isDealer = (mockData.dealerIndex == (int)i);
        // uiSeat.yourTurn = (mockData.actorTurnIndex == (int)i);
        uiSeat.isInHand = mockData.seats[i].isStillActive; /*? (uiSeat.yourTurn ? "Active..." : "Waiting") : "Folded"; */
