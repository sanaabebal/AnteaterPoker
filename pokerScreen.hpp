...
class pokerScreen {
    public:
        ...
        // CODEX FIX: expose a separate dealer-only control for starting the next hand.
        void setStartHandAction(bool enable);

        function<void()> onFold;
        function<void()> onCheck;
        function<void(int)> onBet;
        function<void()> onAllIn;
        function<void()> onStartHand; // CODEX FIX: callback fired when the dealer starts the next hand.
    ...
    private:
        ...
        GtkWidget *startHandButton; // CODEX FIX: button is disabled except for the dealer between hands.
        ...
        static void onStartHandClicked(GtkButton*, gpointer); // CODEX FIX: GTK bridge for dealer next-hand control.
};
