...
void processCharArrBuf(BUF &buf, size_t &index, char (&answer)[n]){
    ...
    // CODEX FIX: require room for the null terminator written after the copied bytes.
    assert(size < (int)n);
    index++;

    // CODEX FIX: clear the destination before copying so shorter names/passwords do not keep old default text.
    memset(answer, 0, n);
    memcpy(answer, buf.data() + index, size);
    // CODEX FIX: terminate at the actual copied length; the sender stores variable-length strings.
    answer[size] = '\0';
    index += size;
}
