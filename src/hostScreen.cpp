...
void hostScreen::playerList(const vector<PLAYER>& players, int mp) {
    ...
    for (auto& p : players) {
        ...
        // CODEX FIX: show slots as 1-based numbers in the host lobby UI.
        gtk_box_pack_start(GTK_BOX(row), cell(to_string(p.playerNum + 1), 60), FALSE, FALSE, 0);
...
void hostScreen::inviteClicked(GtkButton*, gpointer inputData) {
    ...
    // CODEX FIX: force termination after strncpy so host names are sent cleanly.
    ...
    // CODEX FIX: force termination after strncpy so the room password is sent cleanly.
