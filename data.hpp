...
/* Structures and relevant Vectors */
    ...
    struct LoginInfo{
        ...
        char playerName[41]; // holds the player's name
        int playerNum; // represents the player number at the table (numbering starts at 0)
        char password[41]; // set by the player if the host (from client to server), or entered by a joining player if not the host (from server to client)
